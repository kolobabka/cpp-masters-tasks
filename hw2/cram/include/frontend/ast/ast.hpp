...
struct Node : private std::vector<Node *> {
  Node(yy::location loc = yy::location{}) : location_(loc) {}
...
public:
  VarNode(const std::string &name, yy::location loc) : Node(loc), name_(name) {}
...
public:
  DummyNode(yy::location loc = yy::location{}) : Node(loc) {}
...
public:
  ArrayNode(yy::location loc = yy::location{}) : Node(loc) {}
...
public:
  enum class OperType { ADD, SUB, ASSIGN, INPUT, PRINT };
